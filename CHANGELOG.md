...
- **E16 / P1:** `ValueValidator` compares min/eq/max only when both sides are set, and binds `lt`/`gt`/`eq` with `is None` so `0` is kept. `DecimalValidator.value` is typed as `DECIMAL`, not `DEBUG`.
